...
public class controladorProducto {
    ...
    @GetMapping("/categoria/{cate}")
        public ArrayList<producto> categoria(@PathVariable("cate") String categoriaa) {
        if(metodos.mostrarCategoria(categoriaa).size()>0){
            return metodos.mostrarCategoria(categoriaa);
        }
        else{
            return no;
        }
